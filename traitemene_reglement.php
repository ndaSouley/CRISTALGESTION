...
//$data = json_decode($InputJsonString, true);
	...
	//VARIABLES REGLEMNTS DU LOYER LOCATAIRE
		...
		$insquery = "INSERT INTO calendrier_paie(id_locataire,
			...
			if(($_POST['action'])=="REQUET_selecte_locataire") {
				...
				$ps=$DBcon->prepare("SELECT
											...
											$requete1 = "SELECT
																...
																calendrier_paie
																WHERE
																calendrier_paie.id_bien_locataire_calendrier=bien.id_bien
																AND calendrier_paie.id_locataire=locataire.id_locataire
																AND calendrier_paie.id_bien_locataire_calendrier='".$V_id_bien."'
																AND calendrier_paie.id_annee='".$annee."'";
												...
												while($donnees = $resultat1->fetch(PDO::FETCH_ASSOC)) {
													...
													$Janvier = utf8_decode($donnees['Janvier']);
													$Fevrier = utf8_decode($donnees['Fevrier']);
													$Mars = utf8_decode($donnees['Mars']);
													$Avril = utf8_decode($donnees['Avril']);
													$Mai = utf8_decode($donnees['Mai']);
													$Juin = utf8_decode($donnees['Juin']);
													$Juillet = utf8_decode($donnees['Juillet']);
													$Aout = utf8_decode($donnees['Aout']);
													$Septembre = utf8_decode($donnees['Septembre']);
													$Octobre = utf8_decode($donnees['Octobre']);
													$Novembre = utf8_decode($donnees['Novembre']);
													$Decembre = utf8_decode($donnees['Decembre']);
													$id_annee = utf8_decode($donnees['id_annee']);
			...
			if(($_POST['action'])=="REQUET_selecte_nouveau_locataire") {
				$V_id_bien = $_POST['V_id_bien'];
				 //echo"     je suis dans la select avec matricule_collecteur ".$matricule_collecteur ;
				//echo "id_quit = ".$id_quit;
						$json = array();
				...
				echo "{\"Var_id_bien\":\"$nom_locataire\",}";
