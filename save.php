...
//$data = json_decode($InputJsonString, true);
	...
	//VARIABLES REGLEMNTS DU LOYER LOCATAIRE
				...
				while($donnees = $resultat->fetch(PDO::FETCH_ASSOC)) {
					...
					$Mt_charge_req_bon_ok = utf8_decode($donnees['Mt_charge']);
				...
				$msg = "Erreur Insert user";
					...
					if($Loyer_locataire_BD ==''){// DEBUT LOYER LOCATAIRE BD
						...
						if($mt_verse == $mt_total_payer){
							...
							$Result_1 = $mt_verse - $Loyer_locataire; 
							//$Difference = $Result_1 - $penalite;
		...
		$insquery = "INSERT INTO reglement_locataire(id_locataire,
					...
					$stmt->execute(array(":id_locataire" => $REQ_id_locataire,
											...
											":frais_penalite_agence" => $Result_1,
											":Id_mode_paiement" => $mode_reglement,
											":Mt_travaux" => 0,
											":Mt_charge" => 0
		...
		$insquery = "INSERT INTO calendrier_paie(id_locataire,
					...
					$stmt->execute(array(":id_locataire" => $REQ_id_locataire,
								...
								$id_calendrier_reglement_locataire = $DBcon->lastInsertId();
										...
										$SOMME_1 = $mt_verse - $Loyer_locataire;
		...
		$insquery = "INSERT INTO calendrier_paie(id_locataire,
					...
					$stmt->execute(array(":id_locataire" => $REQ_id_locataire,
								...
								$id_calendrier_reglement_locataire = $DBcon->lastInsertId();
													...
													}elseif($SOMME_1>$penalite){
														
														
														$dif_pour_charge =$SOMME_1 - $penalite; 
								...
								$id_calendrier_reglement_locataire = $DBcon->lastInsertId();
									...
									if($charge==''){
										...
										if($penalite == 0){
											...
											$Reste_montant= $mt_verse - $mt_total_payer;
		...
		$insquery = "INSERT INTO reglement_locataire(id_locataire,
					...
					$stmt->execute(array(":Fevrier" => $payer,
											":id_calendrier_paie" => $Calendrier_paie_bon['id_calendrier_paie']
												...
												$calcul_1 = $Loyer_locataire + $penalite;
												
												$reste_1 = $mt_verse - $calcul_1;
		...
		$insquery = "INSERT INTO reglement_locataire(id_locataire,
					...
					$stmt->execute(array(":Fevrier" => $payer,
											...
											$calcul_1 = $Loyer_locataire + $penalite + $charge;
		...
		$insquery = "INSERT INTO reglement_locataire(id_locataire,
				...
				//*************************** FIN DE L'INSERTION DANS LA TABLE 	AVEC DES LOCATAIRE INEEXISTANT********////////////////	
							...
							if($Loyer_locataire_BD == $REQ_LOYER_PROPRIETAIRE){
								...
								if($MT_penalite_req == $frais_penalite_agence){
									
									if($Mt_charge_req_bon_ok == $Mt_travaux_req_Bon_ok){
										
										if($penalite ==0 && $charge == ''){
		...
		$insquery = "INSERT INTO reglement_locataire(id_locataire,
					...
					$stmt->execute(array(":Decembre" => $payer,
										...
										}elseif($Mt_charge_req_bon_ok > $Mt_travaux_req_Bon_ok){
											
											
											$rest_calcul = $Mt_charge_req_bon_ok - $Mt_travaux_req_Bon_ok;
											
											$acompte = $mt_verse - $rest_calcul;
											
											
											
											$insquery = "UPDATE  reglement_locataire  SET Mt_travaux =:Mt_travaux
																				
												
															WHERE id_reglement =:id_reglement";
												
					$stmt = $DBcon->prepare($insquery);
					$stmt->execute(array(":Mt_travaux" => $Mt_travaux_req_Bon_ok+$rest_calcul,
										
											":id_reglement" => $id_reglement
											));	
									
											if($acompte < $Loyer_locataire){
		...
		$insquery = "INSERT INTO reglement_locataire(id_locataire,
					...
					$stmt->execute(array(":Decembre" => $payer,
											...
											}// FIN $charge == $Mt_travaux_req_Bon_ok
									
									
									
									
									}elseif($MT_penalite_req == $frais_penalite_agence){
										...
										}// FIN $MT_penalite_req == $frais_penalite_agence
								
								
								
								
								}// FIN $Loyer_locataire_BD == $REQ_LOYER_PROPRIETAIRE
