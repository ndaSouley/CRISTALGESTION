	...
	//VARIABLES REGLEMNTS DU LOYER LOCATAIRE
				...
				while($donnees = $resultat->fetch(PDO::FETCH_ASSOC)) {
					...
					$requete = "SELECT
										...
										reglement_locataire.Mt_restant AS MT_RESTANT_BD,
										eglement_locataire.frais_penalite_agence
				...
				while($donnees = $resultat->fetch(PDO::FETCH_ASSOC)) {
					...
					$Mt_penalite_BD = utf8_decode($donnees['Mt_penalite']);
					$MT_RESTANT_BD = utf8_decode($donnees['MT_RESTANT_BD']);
					$frais_penalite_agence_BD = utf8_decode($donnees['frais_penalite_agence']);
				...
				$msg = "Erreur Insert user";
					...
					if($Loyer_locataire_BD==''){// DEBUT LOYER LOCATAIRE BD
						...
						if($mt_verse>$Loyer_locataire){//VERIFICATION DU MONTANT VERSE
						
						
						
						
						
						if($penalite=='10%'){//PENALITE 1
		...
		$insquery = "INSERT INTO reglement_locataire(id_locataire,
					...
					$stmt->execute(array(":Decembre" => $payer,
										...
										}elseif($Mt_penalite_BD>$frais_penalite_agence_BD){//$Mt_penalite_BD>$frais_penalite_agence_BD 
										
										$frais_penalite_a_payer=$Mt_Total_restant-$Mt_penalite_BD;
										
										
										
										if($frais_penalite_a_payer<0){//paiement de la penalite
					...
					$stmt->execute(array(":Loyer_locataire" => $Loyer_locataire_BD+$Mt_restant_loyer,
											...
											}elseif($frais_penalite_a_payer>0){
												
											$acompte_loyer=$Mt_Total_restant-$Mt_penalite_BD;
					...
					$stmt = $DBcon->prepare($insquery);
					$stmt->execute(array(":Loyer_locataire" => $Loyer_locataire_BD+$Mt_restant_loyer,
											":frais_penalite_agence" => $Mt_penalite_BD,
		...
		$insquery = "INSERT INTO reglement_locataire(id_locataire,
					...
					$stmt->execute(array(":Loyer_locataire" => $Loyer_locataire_BD+$Mt_restant_loyer,
						...
						}//FIN DE LA BOUCLE IF SI MT RESTANT =0
							
							
							}elseif($Loyer_locataire_BD = $REQ_LOYER_PROPRIETAIRE){
								
								$frais_penalite_a_payer=$mt_verse-$Mt_penalite_BD;
										...
										if($frais_penalite_a_payer<0){//paiement de la penalite
											...
											$insquery = "UPDATE  reglement_locataire  SET 
																						frais_penalite_agence =:frais_penalite_agence
					...
					$stmt->execute(array(":frais_penalite_agence" => $mt_verse,
											...
											$acompte_loyer=$mt_verse-$Mt_penalite_BD;
					...
					$stmt = $DBcon->prepare($insquery);
					$stmt->execute(array(":frais_penalite_agence" => $Mt_penalite_BD,
