...
reglement_locataire.mt_verse_2,
reglement_locataire.Mt_restant_loyer,
...
AND `user`.id_user=reglement_locataire.id_user";
				...
				while($donnees = $resultat1->fetch(PDO::FETCH_ASSOC)) {
					...
					$Mt_verse = utf8_decode($donnees['mt_verse_2']);
					$V_Mt_restant = utf8_decode($donnees['Mt_restant']);
					$prix_bien = utf8_decode($donnees['prix_bien']);
					$V_Mt_restant_loyer = utf8_decode($donnees['Mt_restant_loyer']);
					...
					$V_Loyer_locataire = utf8_decode($donnees['Loyer_locataire']);
...
-->        </td>
      ...
      <tr>
        <td colspan="3"><table width="104%" border="0" align="center" cellspacing="6">
          <tr>
          ...
          <tr>
            <td width="28%" height="46"> Avance:<?php if($Mt_verse==''){
				echo(' ' .number_format($V_Loyer_locataire));
				
				}elseif($Mt_verse!=''){
						echo(' ' .number_format($V_Mt_restant_loyer));
					}?>  FCFA  </td>
      ...
      <tr>
        ...
        <td colspan="2"><?php if($V_Loyer_locataire==$prix_bien){
			
			echo'Solde sur la période de ';
			}elseif($V_Loyer_locataire!=$prix_bien){
				
				echo'Avance reçu sur la période de ' ;
...
des lieux s'il en existe un. </td>
      ...
      <tr>
        <td height="37" valign="top"><p align="center">&nbsp;</p></td>
        </tr>
    </table></td>
  </tr>
</table>
