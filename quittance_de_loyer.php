...
//$date_ipression_ok= gmdate("d-m-Y H:i:s",$date_ipression);
//echo"date impression " . $date_ipression_ok;

$Id_bien=$_GET['Id_bien_envoye'];

$requete1 = "SELECT * FROM reglement_locataire WHERE id_bien='".$Id_bien."'";
				...
				while($donnees = $resultat1->fetch(PDO::FETCH_ASSOC)) {
					...
					$date_reglement = utf8_decode($donnees['date_reglement']);
...
$requete1 = "SELECT
					...
					reglement_locataire.Num_cheque,
					reglement_locataire.nom_banque,
					annee.id_annee,
					annee.annee,
					reglement_locataire.id_annee,
					`user`.id_user,
					...
					AND mode_reglement.id_mode_reglement=reglement_locataire.Id_mode_paiement
					and reglement_locataire.id_bien=bien.id_bien
					and reglement_locataire.date_reglement='".$date_reglement."'
				...
				while($donnees = $resultat1->fetch(PDO::FETCH_ASSOC)) {
					...
					$id_mode_reglement = utf8_decode($donnees['id_mode_reglement']);
          ...
          $id_mois = '02';
					...
					$V_Libelle_mode_reglement = utf8_decode($donnees['Libelle_mode_reglement']);
					$id_reglement = utf8_decode($donnees['id_reglement']);
					$V_Nom_user = utf8_decode($donnees['Nom_user']);
					$V_Num_cheque = utf8_decode($donnees['Num_cheque']);
					$V_id_mode_reglement = utf8_decode($donnees['id_mode_reglement']);
					$V_nom_banque = utf8_decode($donnees['nom_banque']);
					$V_prenoms_user = utf8_decode($donnees['prenoms_user']);
					$V_date_reglement_v1 = utf8_decode($donnees['date_reglement']);
					$V_date_reglement = date("d-m-Y", strtotime($V_date_reglement_v1));
...
<table width="79%" border="0" align="center" cellpadding="0" cellspacing="4">
  <tr>
    <td height="578" valign="top"><table width="90%" border="0" align="center" cellspacing="0">
      <tr>
      ...
      <table width="106%" border="0" align="center" cellpadding="0" cellspacing="1">
      <tr>
        <td height="52" colspan="3"><table width="51%" height="98" align="center" cellspacing="2" bordercolor="#000000">
          <tr>
            <td height="92"><strong><CENTER>
...
-->        </td>
      ...
      <tr>
        <td colspan="4"><table width="104%" border="0" align="center" cellspacing="6">
          <tr>
            ...
            <td width="15%" rowspan="2"><table width="11%" border="0" cellspacing="2">
              <tr>
                <td width="16%" height="37"><h1>N°:<?php echo($id_reglement);?></h1></td>
                <td width="84%" bgcolor="#F1F1F1">&nbsp;</td>
                </tr>
            </table>
              <strong><h4>Exemplaire: Client</h4></strong></td>
          </tr>
          <tr>
            <td height="37" colspan="2">Date d'impression: <?php echo($date_ipression_ok);?></td>
          </tr>
          <tr>
            <td height="37" colspan="2">Date de reception: <?php echo($V_date_reglement);?></td>
          </tr>
          <tr>
      ...
      <tr>
        ...
        <td width="80%" bgcolor="#F1F1F1">Mode de Règlement:<?php echo(' ' . ' <strong>'.$V_Libelle_mode_reglement . ' ' );?> <?php echo( ' ' . ' ' );?>  <?php if($id_mode_reglement==1){
			
			
			}elseif($id_mode_reglement==2){
				echo(' &nbsp ' . '  &nbsp'. '  N°:  &nbsp '  .$V_Num_cheque  .  ' &nbsp ' .  ' &nbsp '   . ' Banque: ' .  ' &nbsp  '  .  $V_nom_banque );
      ...
      <tr>
        ...
        <td valign="top"><p>1 Qu'il n'ait justifié au propriétaire par une quittance du Receveur qu'il a acquitté toutes ses contributions personnelles et mobilières de l'année courante.</p><p>
            2 Qu'il n'ait donné ou reçu congé par écrit dans les délais prescrits</p><p>
            3 Qu'il n'ait fait faire toutes les réparations locatives à sa charge
            suivant l'usage ou d'après l'état
          des lieux s'il en existe un.</p></td>
        <td valign="top" width="18%"><p> </p></td>
        </tr>
      
        <tr>
          <td><p align="center"></p></td>
          <td><strong><u>Signature du Locataire</u></strong></td>
          <td colspan="2"><strong><u>Signature&amp;Cachet</u></strong><u><strong> Cristal</strong></u>&nbsp;</td>
        </tr>
        <tr>
          <td colspan="2"><p align="center"></p>            <p align="center"></p></td>
...
<table width="79%" border="0" align="center" cellpadding="0" cellspacing="4" >
  <tr>
    <td height="578" ><table width="90%" border="0" align="center" cellspacing="0">
      <tr>
        <td width="30%" style="margin-top: 70%; margin-left: 50%; text-align: center;"><strong><img src="css/images/LOGO_CRISTAL_HOME_1.png" width="107" height="137" /></strong></td>
        <td width="60%">&nbsp;</td>
        <td width="13%"><strong><img src="css/images/logo_cristal_ok.png" width="218" height="97" /></strong></td>
...
--></td>
        ...
        <tr>
          <td colspan="4"><table width="99%" border="0" align="center" cellspacing="6">
            <tr>
              <td colspan="2" style="margin-left:20%">N°Appt: <?php echo($num_appartement) ?></td>
              <td colspan="2" rowspan="2">Situation géographique:<?php echo($quartier_bien);?>.</td>
              <td width="21%" rowspan="2"><table width="11%" border="0" cellspacing="2">
                <tr>
                  <td width="16%" height="37"><h1>N°:<?php echo($id_reglement);?></h1></td>
                  <td width="84%" bgcolor="#F1F1F1">&nbsp;</td>
                </tr>
                ...
                <strong>
                  <h4>Exemplaire: Comptabilité</h4>
                </strong></td>
            </tr>
            <tr>
              <td height="20" colspan="2">Date d'impression: <?php echo($date_ipression_ok);?></td>
            </tr>
            <tr>
              <td height="37" colspan="2">Date de reception: <?php echo($V_date_reglement);?></td>
            </tr>
        ...
        <tr>
          <td width="5%" height="31"><br /></td>
          <td width="80%" bgcolor="#F1F1F1">Mode de Règlement:<?php echo(' ' . ' <strong>'.$V_Libelle_mode_reglement . ' ' );?> <?php echo( ' ' . ' ' );?>
            <?php if($id_mode_reglement==1){
        ...
        <tr>
          <td valign="top"><p>&nbsp;</p></td>
          <td valign="top"><p>1 Qu'il n'ait justifié au propriétaire par une quittance du Receveur qu'il a acquitté toutes ses contributions personnelles et mobilières de l'année courante.</p>
            <p> 2 Qu'il n'ait donné ou reçu congé par écrit dans les délais prescrits</p>
            <p> 3 Qu'il n'ait fait faire toutes les réparations locatives à sa charge
              suivant l'usage ou d'après l'état
              des lieux s'il en existe un.</p></td>
          <td valign="top" width="18%"><p> </p></td>
        </tr>
