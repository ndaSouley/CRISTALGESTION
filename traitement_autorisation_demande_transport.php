	...
	$stmt = null;
	            // Variables contribuable	
				 ...
				 $collecteur = htmlspecialchars($_POST['collecteur']);
				  $itineraire = htmlspecialchars($_POST['itineraire']);
				...
				$date_doc = htmlspecialchars($_POST['date_doc']);
				...
				$type_quit = htmlspecialchars($_POST['type_quit']);
				//echo"type quittance".$type_quit;
			 ...
			 if(($_POST['action'])== "INSERT") {
				...
				$insquery = "INSERT INTO quittance(code_quittance,
					...
					$stmt->execute(array(":code_quittance" => $numquittance,
										...
										":date_habilitation" => $datehabilitation,
				...
				$insquery = "INSERT INTO service(id_abonn_serv,
					...
					$stmt->execute(array(":id_abonn_serv" => $id_abonn_serv,
						...
						$stmt->closeCursor();				
				$msgexecute	= "DEMANDE AUTORISATION DE TRANSPORT AJOUTE AVEC SUCCES !";
			...
			$insquery = "UPDATE  contribuable SET
				...
				$insquery = "UPDATE service SET id_abonn_serv = :id_abonn_serv,
					...
					$stmt->execute(array(":id_abonn_serv" => $id_abonn_serv,
						...
						$stmt->closeCursor();				
				//echo "Fin traitement";
					$msgexecute	= "DEMANDE AUTORISATION DE TRANSPORT MODIFIER AVEC SUCCES !";
			$DBcon->commit();
			$DBcon = null;
			//echo "[{\"Etat\":\"SUCCES\",\"Motif\":\"OPERATION EFFECTUEE AVEC SUCCES !\"}]";
			echo "{\"Etat\":\"SUCCES\",\"Motif\":\"$msgexecute\"}";
			...
			if(($_POST['action'])== "SELECT") {
				...
				// requête qui récupère les informations de la facture
					$requete = "SELECT
									...
									INNER JOIN type_quittance ON quittance.Id_Type_quit = type_quittance.Id_Type_quit
				...
				while($donnees = $resultat->fetch(PDO::FETCH_ASSOC)) {
					...
					$montant_macaron = utf8_encode($donnees['montant_macaron']);
					$Id_Type_quit = utf8_encode($donnees['Id_Type_quit']);
					...
					$region = utf8_encode($donnees['region']);
					$code_periode = utf8_encode($donnees['code_periode']);
					$Lib_Type_quit = utf8_encode($donnees['Lib_Type_quit']);
